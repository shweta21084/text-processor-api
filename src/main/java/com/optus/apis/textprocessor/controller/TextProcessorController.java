...
public class TextProcessorController {
	...
	@GetMapping(path = "/top/{limit}", produces= "text/csv")
	public ResponseEntity<?> getHighestCountWordsByLimit(@PathVariable Integer limit,
			@RequestHeader(value = "Trace-Id", defaultValue = "") String traceId) {
		traceId = getTraceId(traceId);
		try {
			String searchAndCount = textProcessorService.getHighestCountWordsByLimit(limit, traceId);
			return new ResponseEntity<>(searchAndCount, HttpStatus.OK);
		} catch (ProcessExecutionException e) {
			return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
		}
